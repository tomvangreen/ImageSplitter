...
public class TextureSplitter {
   ...
   public boolean run() {
      ...
      String updatedFormat = namingFormat.replace("{0}", "%1$s").replace("{1}", "%2$d").replace("{2}", "%3$d");
      for (int y = 0; y < tilesY; y++) {
         for (int x = 0; x < tilesX; x++) {
            prepareOutputImage(image, g, g2, x, y);

            if (canExport(outputImage, tileWidth, tileHeight)) {
               if (!saveOutputImage(nameWithoutExtension, extension, outputImage, updatedFormat, x, y)) {
                  return false;
               }
            }
   ...
   private boolean saveOutputImage(String nameWithoutExtension, String extension, BufferedImage outputImage, String updatedFormat, int x, int y) {
      String indexKey = String.format("%d.%d", x, y);
      String outputFileName = index.get(indexKey);
      if (outputFileName == null || outputFileName == "") {
         outputFileName = String.format(updatedFormat, nameWithoutExtension, x, y);
      }
